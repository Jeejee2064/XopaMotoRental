...
// Overland — see plan notes), so this template has no "remaining balance"
// line. The refundable security deposit is a separate card-hold (AUTH) flow
// an admin triggers after booking — see app/api/pay/auth/route.js — not
// mentioned in this confirmation email since it isn't sent yet at this point.
//
        ...
        <div class="content">
          ...
          <div class="section">
            ...
            <div class="detail-row">
              ...
              <span class="detail-value">${booking.paid
                ? `<span class="highlight">$${Number(booking.total_price).toFixed(2)} PAGADO ✓</span>`
                : `$${Number(booking.total_price).toFixed(2)} — Pendiente de pago`}</span>
            </div>
            <div class="detail-row">
